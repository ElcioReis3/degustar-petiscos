...
// CLIENT COMPONENT — modal de login com Google e convidado.
...
export function LoginDialog({ open, onOpenChange, onLogin }: LoginDialogProps) {
  ...
  // Em produção: troque por signIn("google") do next-auth
  const handleGoogle = () => {
    setLoading(true);
    setTimeout(() => {
      onLogin({
      ...
      <DialogContent className="sm:max-w-sm rounded-2xl">
        ...
        {mode === "guest" && (
          <div className="flex flex-col gap-3 pt-1">
            ...
            >
              <ChevronLeft size={15} className="mr-1" aria-hidden />
              Voltar
            </Button>
            ...
            >
              Continuar
              <ChevronRight size={15} className="ml-1" aria-hidden />
            </Button>
          </div>
        )}
      </DialogContent>
